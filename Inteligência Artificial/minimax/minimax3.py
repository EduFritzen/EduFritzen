...
def verifica_vencedor(estado, player): 
    ...
    if [player, player, player, player] in estado_vitoria:
        return True
    else:
        return False
...
def minimax(estado, profundidade, player, alfa, beta):
    ...
    for celula in posicoes_vazias(estado): # Para cada movimento válido
        ...
        estado[x][y] = player
        score = minimax(estado, profundidade - 1, -player, alfa, beta)
        estado[x][y] = 0
        score[0], score[1] = x, y
        ...
        if player == IA:
            ...
            alfa = max(alfa, melhor[2])
            if alfa >= beta:
                break
        else:
            if score[2] < melhor[2]:
                melhor = score  # min value
            beta = min(beta, melhor[2])
            if alfa <= beta:
                break
...
def jogada_ia(escolha_ia, escolha_h):
    ...
    if profundidade == 16: # primeira jogada da IA
        ...
        movimento = minimax(tabuleiro, profundidade, IA, -infinito, +infinito)
